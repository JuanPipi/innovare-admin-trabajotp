    ...
    <div className="marquee-track w-full overflow-hidden">
      <div className="animate-marquee flex gap-5 w-max">
        {track.map((e, i) => (
          <div
            key={i}
            className="flex flex-col items-center text-center rounded-xl p-6 border border-white/10 bg-white/5 w-52 shrink-0"
          >
            <div className="w-12 h-12 rounded-full bg-blue-glow/15 flex items-center justify-center mb-3">
              <e.icon className="text-blue-glow" size={22} />
            </div>
            <h4 className="font-sans text-sm font-bold text-primary-foreground mb-1">{e.label}</h4>
            <p className="text-primary-foreground/55 text-xs leading-relaxed">{e.desc}</p>
          </div>
        ))}
      </div>
    </div>
