...
const MethodologySection = () => {
  ...
  return (
    <section id="metodologia" className="py-24 md:py-32 bg-background/80">
          ...
          {steps.map((s, i) => (
            <ScrollReveal key={s.num} delay={i * 150}>
              <div className={`relative flex flex-col items-center text-center transition-all duration-500 ${active === i ? "md:scale-105" : ""}`}>
                <div className={`relative z-10 w-20 h-20 rounded-full bg-card border-2 flex items-center justify-center mb-6 transition-all duration-500 ${
                  active === i ? "border-blue-accent shadow-[0_0_28px_8px_rgba(52,168,83,0.22)]" : "border-blue-accent/30 shadow-card"
                }`}>
                  <span className={`font-bold text-2xl font-sans transition-colors duration-500 ${active === i ? "text-blue-accent" : "text-blue-accent/40"}`}>
                    {s.num}
                  </span>
                </div>
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-4 transition-all duration-500 ${active === i ? "bg-blue-accent" : "bg-blue-accent/10"}`}>
                  <s.icon className={`transition-colors duration-500 ${active === i ? "text-white" : "text-blue-accent"}`} size={20} />
                </div>
                <h3 className={`font-sans text-lg font-bold mb-2 transition-colors duration-500 ${active === i ? "text-blue-accent" : "text-primary"}`}>
                  {s.title}
                </h3>
                <p className={`text-sm leading-relaxed max-w-xs transition-colors duration-500 ${active === i ? "text-foreground" : "text-muted-foreground"}`}>
                  {s.desc}
                </p>
              </div>
            </ScrollReveal>
